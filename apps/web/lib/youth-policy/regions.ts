// 전국 시/도 → 시/군/구 데이터. 지역 선택을 서울·경기로 한정하지 않고 전국으로 확장.
// (세종은 단층제라 하위 시군구 없음 → districts 빈 배열)
...
export const SIDO_NAMES: readonly string[] = REGIONS.map((r) => r.name);

export function getDistricts(sido: string): readonly string[] {
  return REGIONS.find((r) => r.name === sido)?.districts ?? [];
}
